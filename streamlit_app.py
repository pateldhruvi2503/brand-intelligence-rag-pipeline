...
class RAGConfig:
    ...
    vectorstore_path: str = "intel_vectorstore"

config = RAGConfig()

# ── Page setup ────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Intel Brand Intelligence",
    page_icon="🔵",
    layout="wide"
)
...
# ── Sidebar ───────────────────────────────────────────────────────────
with st.sidebar:
    st.image("https://upload.wikimedia.org/wikipedia/commons/7/7d/Intel_logo_%282006-2020%29.svg", width=120)
    st.markdown("## Intel Brand Intelligence")
...
def load_everything():
    ...
    # Build RAG chain
    prompt_template = """
You are an expert brand analyst for Intel. Use ONLY the data provided.
...
def pandas_answer(llm, dataframes, question):
    ...
    pandas_prompt = f"""
You are an expert Intel brand analyst. Answer using the data below.
...
def get_answer(llm, dataframes, qa_chain, question):
    ...
    if route == "PANDAS":
        ...
        combine_prompt = f"""
You are an Intel brand analyst. Combine these analyses into one clear answer.
...
# ── Main app ──────────────────────────────────────────────────────────
st.title("Intel Brand Intelligence Chatbot")
st.caption("Ask anything about Intel's brand data — awareness, consideration, purchase intent, campaigns, signals")

# Load everything
with st.spinner("Loading Intel data and AI models..."):
...
with st.expander("Brand Health Trends", expanded=True):
    try:
        ...
        fig = px.line(
            ...
            color="construct",
            markers=True,
            title="Intel Brand Tracker — All Constructs Over Time",
...
# ── Chat interface ────────────────────────────────────────────────────
st.markdown("### Ask the Intel Brand Analyst")
...
if "sample_q" in st.session_state:
    ...
    prompt = st.chat_input("Ask about Intel's brand data...")
...
if prompt:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analysing Intel data..."):
